...
@Component({
  selector: 'app-productos-niveles',
  templateUrl: './productos-niveles.component.html',
})
export class ProductosnivelesComponent implements OnInit {
  constructor(
  ...
  Remplazar() {
    if (confirm('Esta seguro de remplazar los cambios? ')) {
      this.productosservice.UpdateNiveles(this.niveles,this.idusuario,this.token_).subscribe({
        next: (data) => {
